...
time_opener = '<span class="meta">'
sendertime_closer = '</span>'
text_opener = '<p>'
text_closer = '</p>'
...
output_filename = 'initial_output.csv'
...
for input_filename in input_filenames:
    ...
    names = (names.replace(names_start, '').replace(', ', ',')
             .replace(chat_leave_message, '').strip())
    ...
    while has_next_message:
        ...
        index, sender = get_data(content, sender_opener, sendertime_closer, index)
        index, time = get_data(content, time_opener, sendertime_closer, index)
